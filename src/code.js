class AlaORMGS {
  ...
  /**
   * Selects specific columns from the table with optional filtering.
   * @param {string} columns - Comma-separated column names to retrieve.
   * @param {string} [whereClause] - Optional WHERE clause condition (e.g., "id = 1").
   * @returns {Array<Object>} - Array of objects with selected columns.
   */
  select(columns, whereClause = null) {
    return this._measureExecutionTime(() => {
      let columnArray = columns.split(',').map(col => col.trim());

      // Validate if all requested columns exist
      ...
      if (whereClause) {
        let query = `SELECT * FROM ? WHERE ${whereClause}`;
        this.log(query);
        filteredData = this.alasql(query, [this.jsonData]);
      }

      return filteredData.map(row => {
        let obj = {};
        columnArray.forEach(col => {
          obj[col] = row[this.columns.indexOf(col)];
        });
        return obj;
      });
    }, 'select');
